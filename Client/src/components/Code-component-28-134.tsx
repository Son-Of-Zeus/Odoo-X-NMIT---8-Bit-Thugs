...
export function EditProfileDialog({ isOpen, onClose, userProfile, onSave }: EditProfileDialogProps) {
  ...
  const [profileImages, setProfileImages] = useState<string[]>(userProfile.avatar ? [userProfile.avatar] : []);
  ...
  const handleImageUpload = async (files: File[]) => {
    ...
    try {
      ...
      // Create mock URLs for the uploaded images
      const newImageUrls = files.map((file, index) => {
        return URL.createObjectURL(file);
      });
      
      // For profile, we only want one image, so take the first one
      const newAvatarUrl = newImageUrls[0];
      setProfileImages([newAvatarUrl]);
      setFormData(prev => ({
        ...prev,
        avatar: newAvatarUrl
      }));
  ...
  const handleCancel = () => {
    ...
    setProfileImages(userProfile.avatar ? [userProfile.avatar] : []);
    onClose();
  };
